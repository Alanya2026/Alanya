...
- **i18n** et **validation des formulaires** : **à faire** (chantiers §7 et §8).
...
| Écran | Lignes | Note |
|---|---|---|
| `authentification/login_screen.dart` | 217 | + validation (§8) |
| `authentification/signup_screen.dart` | 229 | + validation, corriger « Create Account » → FR/i18n |
| `authentification/forgot_password_screen.dart` | 333 | étapes OTP + validation |
| `profile/profile_screen.dart` | 787 | |
| `profile/settings_screen.dart` | 240 | **y mettre le sélecteur de thème (§6)** |
| `profile/edit_profile_screen.dart` | 388 | + validation |
| `profile/preferred_contacts_screen.dart` | 180 | |
| `contacts/contacts_screen.dart` | 94 | ⚠ AppBar indigo incohérente + **données factices** à brancher |
...
Constat : chaînes mêlant FR et EN (« Create Account », « Select Contact »,
« Unknown »…), aucune localisation.
...
flutter:
  generate: true
```
2. `l10n.yaml` à la racine :
```yaml
arb-dir: lib/l10n
template-arb-file: app_fr.arb
output-localization-file: app_localizations.dart
```
3. `lib/l10n/app_fr.arb` (+ `app_en.arb`), puis `flutter gen-l10n`.
4. `MaterialApp` : `localizationsDelegates: AppLocalizations.localizationsDelegates`,
   `supportedLocales: AppLocalizations.supportedLocales`.
5. **Migration progressive** : remplacer les chaînes par
   `AppLocalizations.of(context)!.xxx`, **français par défaut**, anglais en 2ᵉ
   locale. Combiner avec la passe UI de chaque écran. Corriger les libellés EN
   résiduels (signup « Create Account », `contacts_screen` « Select Contact », etc.).
