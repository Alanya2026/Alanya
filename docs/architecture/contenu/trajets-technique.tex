...
\thd{Battement} \\
\midrule
Nominal  & à plus de 10 min de l'ETA         & \code{balanced} & 75 m & 15 s & 90 s \\
Approche & à moins de 10 min, ou 1 km du but & \code{high}     & 30 m &  8 s & 30 s \\
...
\midrule
\code{TRIP\_FILTER\_M\_NOMINAL}    & 75 m  & Trace plus anguleuse en ville~; sans effet sur
                                            la sûreté. \\
\code{TRIP\_FLOOR\_S\_NOMINAL}     & 15 s  & Le pin saccade. C'est la molette la plus sûre à
                                            tourner~: elle ne change que le nombre de
                                            paquets. \\
\code{TRIP\_BEAT\_S\_NOMINAL}      & 90 s  & Un arrêt met plus longtemps à se confirmer, et
                                            le seuil de péremption recule d'autant. \\
\code{TRIP\_FILTER\_M\_APPROACH}   & 30 m  & L'arrivée se détecte plus tard. \\
\code{TRIP\_FLOOR\_S\_APPROACH}    & 8 s   & --- \\
\code{TRIP\_BEAT\_S\_APPROACH}     & 30 s  & --- \\
...
\begin{enumerate}[leftmargin=1.6em]
  \item être à moins de \code{dest\_radius\_m} (150~m par défaut)~;
...
\begin{note}[Pourquoi 150 m, et pas 50]
...
déclencherait à un pâté de maisons. L'hystérésis et le portillon de vitesse font le reste du
travail~: passer devant sa rue sans s'arrêter est le faux positif numéro un.
\end{note}
